...
(defn process-one-sec
  ...
  (let [done-workers (filter #(done? % time-offset) workers)
        available-workers (into (sorted-map) (concat done-workers (filter idle? workers)))
        ...
        busy-workers (->> (apply dissoc workers (keys available-workers))
                          ...
                          (into (sorted-map)))
        steps-in-progress (map #(last (second %)) busy-workers)
        completed-steps (map #(last (second %)) done-workers)
        updated-accumulator (apply conj accumulator completed-steps)
        updated-eval-set (if (empty? evaluation-set)
                           (apply sorted-set (set/difference (set (keys forward-links)) (set (keys back-links))))
                           (if (not= accumulator updated-accumulator)
                             (set/union (apply disj evaluation-set completed-steps)
                                        (apply sorted-set (apply concat (map forward-links completed-steps))))
                             evaluation-set))
        next-steps (if (not= evaluation-set updated-eval-set)
                     (->> (filter #(let [requirements (set (% back-links))]
                                     (or (nil? requirements)
                                         (= requirements
                                            (set/intersection requirements (set updated-accumulator))))) updated-eval-set)
                          (remove #(contains? (set steps-in-progress) %))
                          sort)
                     '())
        ...
        unused-workers (->> (into {} (apply dissoc available-workers (keys assigned-workers)))
...
(defn part2
  ([data num-workers time-offset]
   (let [back-links (links data second first)
         forward-links (links data first second)
         seconds-states
         (->> (take-while #(not= (set (flatten (seq forward-links))) (set (:accumulator %)))
                          (iterate process-one-sec {:accumulator    []
                                                    ...
                                                    :time-offset    time-offset}))
              vec)]
     (->> (conj seconds-states (process-one-sec (last seconds-states)))
          count
          ((comp dec dec)))))
  ([]
